...
-- First, ensure auth.users table exists (it should be created by Supabase)
-- We'll reference it but not create it
...
-- Articles table - fix foreign key reference to use profiles instead of users
...
-- Content views tracking - fix foreign key reference
...
DROP POLICY IF EXISTS "Premium videos require subscription" ON videos;
...
DROP POLICY IF EXISTS "Only admins can manage article tags" ON article_tags;
DROP POLICY IF EXISTS "Video tags are viewable by everyone" ON video_tags;
DROP POLICY IF EXISTS "Only admins can manage video tags" ON video_tags;
...
-- Create user signup trigger only if it doesn't exist
DO $$
BEGIN
...
EXCEPTION
  WHEN OTHERS THEN
    -- Ignore errors (like duplicate entries)
    NULL;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;